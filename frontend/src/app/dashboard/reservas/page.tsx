...
import { recentBookings, statusStyles, parseCheckIn, parseCheckOut, type Booking } from '@/lib/dashboard-data'
...
export default function ReservasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.sort((a, b) => {
      const dateA = new Date(`2026/${a.checkIn.replace('/', '/')}`).getTime()
      const dateB = new Date(`2026/${b.checkIn.replace('/', '/')}`).getTime()
      return dateB - dateA
    })
  }, [activeTab, search])

  const counts = useMemo(() => {
    const all = recentBookings.length
  ...
  const calendarEvents = useMemo(
    () =>
      filtered.map((b) => ({
        id: b.id,
        title: b.guest,
        start: parseCheckIn(b.checkIn),
        end: parseCheckOut(b.checkOut),
        backgroundColor: eventColors[b.status].bg,
        borderColor: eventColors[b.status].border,
        textColor: '#fff',
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-secondary">{recentBookings.length} no total</p>
        </div>
        ...
        <>
                    ...
                    {filtered.map((booking) => (
                      ...
                      >
                        <td className="py-3.5 px-4">
                          <Link href={`/dashboard/imoveis/${booking.propertyId}`} className="flex items-center gap-2.5 hover:text-primary transition-colors" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div>
                              ...
                              <span className="block text-[11px] text-text-secondary">{booking.id}</span>
                            </div>
                          </Link>
                        </td>
                        <td className="py-3.5 px-4 text-text-secondary">{booking.property}</td>
                        <td className="py-3.5 px-4 text-text-secondary">{booking.checkIn}</td>
                        <td className="py-3.5 px-4 text-text-secondary">{booking.checkOut}</td>
                        <td className="py-3.5 px-4">
                ...
                {filtered.map((booking) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <Link href={`/dashboard/imoveis/${booking.propertyId}`} className="flex items-center gap-2.5 min-w-0" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-text-primary truncate">{booking.guest}</p>
                          <p className="text-xs text-text-secondary truncate">{booking.id}</p>
                        </div>
                      </Link>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap flex-shrink-0 ${statusStyles[booking.status]}`}>
                        {statusLabel[booking.status]}
                      </span>
                    </div>
                    <div className="ml-11 space-y-1 text-xs text-text-secondary">
                      ...
                      <p className="font-semibold text-text-primary">R$ {booking.amount.toLocaleString('pt-BR')}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {filtered.length > 0 && (
            <p className="text-xs text-text-secondary text-center">
              Exibindo {filtered.length} de {recentBookings.length} reservas
            </p>
          )}
        </>
      )}
